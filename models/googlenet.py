...
class GoogLeNet(ImgClfModel):
    ...
    def create_model(self, input, options):
        ...
        # Inception (3a)
        conv2d_1 = conv2d(out, num_outputs=64,
                    kernel_size=[1,1], stride=1, padding="SAME",
                    activation_fn=tf.nn.relu)

        conv2d_2 = conv2d(out, num_outputs=96,
                    kernel_size=[1,1], stride=1, padding="SAME",
                    activation_fn=tf.nn.relu)
        ...
        conv2d_3 = conv2d(out, num_outputs=16,
                    kernel_size=[1,1], stride=1, padding="SAME",
                    activation_fn=tf.nn.relu)
        ...
        conv2d_4 = max_pool2d(out, kernel_size=[3,3], stride=1)
        conv2d_4 = conv2d(conv2d_4, num_outputs=32,
                    kernel_size=[1,1], stride=1, padding="SAME",
                    activation_fn=tf.nn.relu)
        inception_1 = tf.concat(3, [conv2d_1, conv2d_2, conv2d_3, conv2d_4])
